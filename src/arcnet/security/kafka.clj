...
(defn producer-config
  "Creates a complete Kafka producer configuration with security.
...
(defn create-producer
  "Creates a new Kafka producer with secure configuration.
...
(defn consumer-config
  "Creates a complete Kafka consumer configuration with security.
...
(defn create-consumer
  "Creates a new Kafka consumer with secure configuration.
...
(defn build-kafka-config
  "Builds a complete Kafka configuration from ARCNet components.
...
(defn test-connection
  "Tests Kafka connectivity with the provided configuration.
  ...
  [config test-topic]
  (log/info "Testing Kafka connection" {:topic test-topic})
  (try
    (with-open [producer (create-producer config)]
    ...
    (catch Exception e
      (log/error e "Kafka connection test failed")
      {:success false
       :error (.getMessage e)})))
